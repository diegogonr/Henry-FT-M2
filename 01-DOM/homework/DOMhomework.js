...
function buildToDo (todo, index){             //{ todo: ToDo{description:'hola', complete: false} , index
  ...
  toDoText.id = index                                //<span id = index>hola</span>

  if(todo.complete) {
    toDoText.className = 'completeText'                     //agrega id
  }

  toDoShell.appendChild (toDoText)                  //agregar un hijo
    // <div class"toDoShell> 
    //   <span id =0>hola</span>
    // </div>
...
function buildToDos(toDos) {
  ...
  let array_objetoTodos = toDos.map((toDo, index) => buildToDo(toDo, index))
  return array_objetoTodos
}
    // [<div class"toDoShell> 
    //   <span id =0>   </span>
    // </div>,
...
function displayToDos() {
  ...
  array_buildToDos.forEach((element) => toDoContainer.appendChild (element) )
}
...
function addToDo() {
  ...
  let objetoToDo_new = new ToDo(toDoInput.value); //// ToDo{description: " " , compelte: false}
  toDoItems.push (objetoToDo_new) 
  toDoInput.value = ""
  displayToDos()
}
...
Button.addEventListener ('click', addToDo)  //solo se pasa la funcion sin invocar para no obtener su valor de retorno, ya que esta no tiene
...
function completeToDo(event) {
  ...
  // Tu código acá:
  toDoItems[index].completeToDo() //se ejecuta el metodo  que cambia el estado a true
  displayToDos()                  //actualizar las tarjetas
}
